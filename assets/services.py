...
class ImageGenerationService:
    ...
    def generate(
        ...
        if not self.api_token:
            raise ValueError(
                "HUGGINGFACE_API_TOKEN non configuré dans les variables d'environnement"
            )
        ...
        try:
            ...
            raise Exception(
                "Timeout lors de la génération de l'image. Le service est peut-être surchargé."
            )
...
class SoundGenerationService:
    ...
    def generate_text_to_speech(
        self, text: str, lang: str = "fr", slow: bool = False
    ) -> Tuple[bytes, dict]:
        """
    ...
    def generate_ambient_music(
        self, description: str, duration: int = 15
    ) -> Tuple[bytes, dict]:
        """
